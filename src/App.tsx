    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <GithubIcon size={18} />
          <h3>
            Set source — <span className="modal-addon-name">{addon.name}</span>
          </h3>
        ...
        <p className="modal-hint">
          Paste a GitHub URL or <code>owner/repo</code> slug.
        </p>
        <input
...
function AddAddonModal({
  ...
  useEffect(() => {
    const cleanup = api.onInstallProgress(({ message }) =>
      setProgress(message),
    );
    return cleanup;
  }, []);
  ...
  return (
    <div className="modal-overlay" onClick={!progress ? onClose : undefined}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="modal-hint">
          Paste a GitHub URL or <code>owner/repo</code> slug. The addon will be
        ...
        <input
          ...
          disabled={!!progress}
          onKeyDown={(e) => {
            if (e.key === "Enter" && slug && !progress) handleInstall();
          }}
        />
        {slug && !progress && (
          <div className="repo-preview">
...
function SortIcon({
  ...
  if (col !== sortCol)
    return <ChevronsUpDown size={11} className="sort-icon sort-icon-idle" />;
  return sortDir === "asc" ? (
...
export default function App() {
  ...
  useEffect(() => {
    api.loadConfig().then(async (cfg) => {
      setConfig(cfg);
      if (!cfg.addonsPath) return;
      setLoading(true);
      setScanError(null);
      const res = await api.scanAddons(cfg.addonsPath);
      setLoading(false);
      if (res.success) {
        setAddons(res.addons);
        setSelected(new Set());
    ...
    const cleanup = api.onUpdateProgress(({ addonName, message }) => {
      setAddons((prev) =>
        prev.map((a) =>
          a.name === addonName ? { ...a, progressMessage: message } : a,
        ),
      );
    });
  ...
  const updateAddon = async (addon: Addon) => {
    ...
    if (res.success) {
      updateAddonStatus(addon.name, {
        status: "up-to-date",
        version: res.version || addon.version,
        latestVersion: res.version,
        progressMessage: undefined,
        foldersUpdated: res.foldersUpdated,
        updateMethod: res.method as "git" | "zip" | undefined,
      });
    } else {
      updateAddonStatus(addon.name, {
        status: "error",
  ...
  const removeAddon = async (addon: Addon) => {
    ...
    const confirmed = window.confirm(
      `Remove "${addon.name}" and delete its folder? This cannot be undone.`,
    );
    if (!confirmed) return;
    const res = await api.removeAddon({
  ...
  const installAddon = async (repoSlug: string) => {
    if (!config.addonsPath) return;
    const res = await api.installAddon({
      addonsPath: config.addonsPath,
      repoSlug,
    });
    if (res.success) {
      setAddingAddon(false);
      doScan(config.addonsPath);
    }
    return res;
  };

  const switchAddonMode = async (
    addon: Addon,
    targetMode: "releases" | "git",
  ) => {
    if (!addon.repoSlug) return;
    updateAddonStatus(addon.name, {
      status: "updating",
      errorMessage: undefined,
      progressMessage: "Switching mode…",
    });
  ...
  const togglePin = async (addon: Addon) => {
    ...
    updateAddonStatus(addon.name, {
      pinned,
      status: pinned ? "idle" : addon.status,
    });
  };
  ...
  const setSource = async (addon: Addon, source: string) => {
    ...
    const normalizedSource =
      slug.includes("/") && !slug.includes("github.com")
        ? `https://github.com/${slug}`
        : source;
    await api.setAddonSource({
      addonsPath: config.addonsPath,
      addonName: addon.name,
    ...
    updateAddonStatus(addon.name, {
      source: normalizedSource,
      repoSlug: slug.includes("/") ? slug : null,
      hasSource: true,
      status: "idle",
    });
  };
  ...
  const filtered = addons
    ...
    .sort((a, b) => {
      ...
      if (sortCol === "source")
        cmp = (a.repoSlug || a.source || "").localeCompare(
          b.repoSlug || b.source || "",
        );
      if (sortCol === "status")
        cmp =
          (STATUS_ORDER[a.pinned ? "idle" : a.status] ?? 9) -
          (STATUS_ORDER[b.pinned ? "idle" : b.status] ?? 9);
      return sortDir === "asc" ? cmp : -cmp;
    });
  const withSource = addons.filter((a) => a.repoSlug).length;
  const updateAvailable = addons.filter(
    (a) => a.status === "update-available",
  ).length;
      ...
      {config.addonsPath && (
        <div className="toolbar">
          ...
          <div className="toolbar-stats">
            ...
            {updateAvailable > 0 && (
              <span className="stat stat-update">
                <Download size={12} />
                {updateAvailable} updates
              </span>
            )}
            {pinned > 0 && (
              <span className="stat stat-pinned">
                <Pin size={12} />
                {pinned} pinned
              </span>
            )}
          ...
          <div className="toolbar-actions">
            ...
            <button
              ...
              onClick={updateAll}
              disabled={
                loading || (selected.size === 0 && updateAvailable === 0)
              }
            >
              <Download size={14} />
              Update{" "}
            ...
            <button
              className="btn btn-ghost"
              onClick={() => api.openFolder(config.addonsPath)}
              title="Open AddOns folder in file manager"
            >
              <FolderOpen size={14} />
              Open folder
            </button>
      ...
      {!config.addonsPath && (
        <div className="empty-state">
          ...
          <p className="empty-sub">
            Point the app at your Ascension AddOns directory to get started.
          </p>
              ...
              {filtered.map((addon) => (
                ...
                >
                  <td className="col-check">
                    <input
                      type="checkbox"
                      checked={selected.has(addon.name)}
                      onChange={() => toggleSelect(addon.name)}
                    />
                  </td>
                  ...
                  <td className="col-version">
                    ...
                    {addon.latestVersion &&
                      addon.latestVersion !== addon.version && (
                  ...
                  <td className="col-source">
                    {addon.repoSlug && (
                      <select
                        ...
                        onChange={(e) =>
                          switchAddonMode(
                            addon,
                            e.target.value as "releases" | "git",
                          )
                        }
                        title="Tracking mode"
                      >
                    ...
                    {addon.isGitRepo && !addon.repoSlug && (
                      <span
                        className="git-badge"
                        title="This folder is a git repository — updates use git pull"
                      >
                  ...
                  <td className="col-actions">
                    <div className="action-group">
                      <button
                        ...
                        onClick={() =>
                          api.openFolder(`${config.addonsPath}/${addon.name}`)
                        }
                      >
                        <FolderOpen size={15} />
                      </button>
                      ...
                      {(addon.repoSlug || addon.isGitRepo) && (
                        <>
                          ...
                          <button
                            ...
                            title={
                              addon.pinned
                                ? "Pinned — updates blocked"
                                : "Update now"
                            }
                            disabled={
                              addon.pinned || addon.status === "updating"
                            }
